...
require('./../styles/mermaidtool.scss');
...
class MermaidTool {
    ...
    parse(mermaidCode, preview, container) {
        this.mermaidCode=mermaidCode;
        preview.classList.remove('mermaid-preview-error');
        try {
            var id = generateId('svg-');
            mermaid.render(id, mermaidCode, (svg) => {
                preview.innerHTML = '';
    ...
    render() {
        ...
        input.value = this.mermaidCode ? this.mermaidCode : '';
        if (input.value) {
            // wait for mermaid
            setTimeout(() => {
    ...
    validate(savedData) {
        return savedData.mermaidCode.trim() !== '';
    }

    save(blockContent) {
        return {
            'mermaidCode': this.mermaidCode,
            'caption': blockContent.querySelector('.mermaid-caption')?.innerText || this.caption,
        }
    }
}
